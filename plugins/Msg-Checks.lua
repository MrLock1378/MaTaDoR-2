...
local lang = redis:get(hash)
  ...
  tdcli.sendMessage(msg.to.id, "", 0, "_🔥This Is Not One Of spłÐΞЯ *Groups* Channel:@LockerTeam 🔥", 0, "md")
...
return
   ...
   del_msg(chat, msg.id)
    kick_user(user, chat)
   if not lang then
  tdcli.sendMessage(chat, msg.id, 0, "_User_ "..user_name.." `[ "..user.." ]` _💥has been_ *kicked* _because of_ *flooding*💥", 0, "md")
   elseif lang then
  tdcli.sendMessage(chat, msg.id, 0, "_کاربر_ "..user_name.." `[ "..user.." ]` _به دلیل ارسال پیام های مکرر اخراج شد💥_", 0💥, "md")
    end
